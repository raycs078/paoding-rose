...
/**
 * {@link RoutingConnectionImpl} 由 {@link RoutingDataSource}
 * 创建，封装对散库系统中的数据库连接抽象。
 * 
 * @author qieqie
 * 
 */
public class RoutingConnectionImpl implements RoutingConnection {
     ...
     */
    public RoutingConnectionImpl(RoutingDataSource routingDataSource) {
     ...
     * @param password
     */
    public RoutingConnectionImpl(RoutingDataSource routingDataSource, String username,
            String password) {
        this.routingDataSource = routingDataSource;
    ...
    protected Connection loadConnection(boolean create) throws SQLException {
        if (realConnection == null && create) {
            realConnection = routingDataSource.getConnectionLocator().getConnection(this);
            if (realConnection == this) {
    ...
    protected void applySettings(Connection real) throws SQLException {
        ...
        for (Map.Entry<Object, Object> entry : clientInfo.entrySet()) {
            ...
            if (!name.startsWith("net.paoding.rose")) {
        ...
        if (transactionIsolation != null) {
            real.setTransactionIsolation(transactionIsolation);
    ...
    public void setHoldability(int holdability) throws SQLException {
        loadConnection(false).setHoldability(holdability);
    }

    @Override
    public int getHoldability() throws SQLException {
        return loadConnection(false).getHoldability();
    }
    ...
    public void setClientInfo(Properties properties) throws SQLClientInfoException {
        if (this.realConnection != null) {
            ...
            if (properties.get(CATALOG) != null) {
                setCatalog(properties.getProperty(CATALOG));
            }
        }
    }
