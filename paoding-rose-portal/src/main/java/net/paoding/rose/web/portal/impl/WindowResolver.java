...
public class WindowResolver implements ParamResolverBean {
    ...
    public Object resolve(Class<?> parameterType, int replicatedCount, int indexOfReplicated,
        ...
        if (PortalResponse.class.isInstance(inv.getResponse())) {
            PortalResponse portalResponse = (PortalResponse) inv.getResponse();
            return portalResponse.getWindow();
        } else {

            return new Window((Portal) null, parameterName, inv.getRequestPath().getUri());
        }
    }
}
